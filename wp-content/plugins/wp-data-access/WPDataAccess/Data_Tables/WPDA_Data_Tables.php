...
namespace WPDataAccess\Data_Tables {
	...
	class WPDA_Data_Tables {
		 ...
		 *
		 */
		public function show( $pub_id, $table_name, $column_names, $responsive, $responsive_cols, $responsive_type, $responsive_icon ) {
			...
			if ( strpos( $table_name, '.' ) ) {
				// Querying tables in other schema's is not allowed!
				return '<p>' . __( 'ERROR: Table not found', 'wp-data-access' ) . '</p>';
			}
			...
			return
				"<table id=\"" . esc_attr( $table_name ) . "\" class=\"display nowrap\" cellspacing=\"0\">" .
				"	<thead>" . $this->show_header( $columns, $responsive, $responsive_cols ) . "</thead>" .
				"	<tfoot>" . $this->show_header( $columns, $responsive, $responsive_cols ) . "</tfoot>" .
				"</table>" .
		...
		protected function show_header( $columns, $responsive, $responsive_cols ) {
			...
			foreach ( $columns as $column ) {
				...
				$html_output .= "<th class=\"$class\">$column</th>";
				$count ++;
			}
